...
func runConfigManagementPluginSidecars(ctx context.Context, appPath, repoPath, pluginName string, envVars *v1alpha1.Env, q *apiclient.ManifestRequest, creds git.Creds, tarDoneCh chan<- bool, tarExcludedGlobs []string) ([]*unstructured.Unstructured, error) {
	...
	tarIncludeFileGlobs := q.ApplicationSource.Plugin.TarIncludeFileGlobs
	...
	cmpManifests, err := generateManifestsCMP(ctx, appPath, repoPath, env, cmpClient, tarDoneCh, tarExcludedGlobs, tarIncludeFileGlobs)
	if err != nil {
...
// manifests.
func generateManifestsCMP(ctx context.Context, appPath, repoPath string, env []string, cmpClient pluginclient.ConfigManagementPluginServiceClient, tarDoneCh chan<- bool, tarExcludedGlobs []string, tarIncludeFileGlobs []string) (*pluginclient.ManifestResponse, error) {
	...
	err = cmp.SendRepoStream(generateManifestStream.Context(), appPath, repoPath, generateManifestStream, env, tarExcludedGlobs, tarIncludeFileGlobs, opts...)
	if err != nil {
...
func (s *Service) GetAppDetails(ctx context.Context, q *apiclient.RepoServerAppDetailsQuery) (*apiclient.RepoAppDetailsResponse, error) {
	...
	operation := func(repoRoot, commitSHA, revision string, ctxSrc operationContextSrc) error {
		...
		case v1alpha1.ApplicationSourceTypePlugin:
			...
			if err := populatePluginAppDetails(ctx, res, opContext.appPath, repoRoot, q, s.gitCredsStore, s.initConstants.CMPTarExcludedGlobs, q.Source.Plugin.TarIncludeFileGlobs); err != nil {
...
func populatePluginAppDetails(ctx context.Context, res *apiclient.RepoAppDetailsResponse, appPath string, repoPath string, q *apiclient.RepoServerAppDetailsQuery, store git.CredsStore, tarExcludedGlobs []string, tarIncludeFileGlobs []string) error {
	...
	err = cmp.SendRepoStream(generateManifestStream.Context(), appPath, repoPath, generateManifestStream, env, tarExcludedGlobs, tarIncludeFileGlobs)
	if err != nil {
